...
<section class="relative pt-20 pb-12 overflow-hidden">
    <div class="absolute inset-0 bg-gradient-to-br from-red-900/40 via-gray-900 to-black"></div>
    <div class="relative container mx-auto px-4 text-center">
        <h1 class="text-4xl md:text-6xl font-bold mb-4">
            <span class="text-white">Daftar</span>
            <span class="bg-gradient-to-r from-red-500 to-orange-500 bg-clip-text text-transparent">Film</span>
        </h1>
    ...
    <div class="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-700">
        <form method="GET" action="{{ route('films.index') }}" id="filterForm">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                ...
                <div class="md:col-span-2">
                    <div class="relative">
                        ...
                        <input type="text" 
                               ...
                               class="w-full bg-gray-900 border border-gray-700 rounded-xl py-3 pl-12 pr-4 text-white focus:outline-none focus:border-red-500 transition">
                    </div>
                </div>
                
                <!-- Genre Filter -->
                <div>
                    <select name="genre" class="w-full bg-gray-900 border border-gray-700 rounded-xl py-3 px-4 text-white focus:outline-none focus:border-red-500">
                ...
                <!-- Status Filter -->
                <div>
                    <select name="status" class="w-full bg-gray-900 border border-gray-700 rounded-xl py-3 px-4 text-white focus:outline-none focus:border-red-500">
                ...
                <!-- Sort -->
                <div>
                    <select name="sort" class="w-full bg-gray-900 border border-gray-700 rounded-xl py-3 px-4 text-white focus:outline-none focus:border-red-500">
                ...
                <!-- Submit Button -->
                <div>
                    <button type="submit" class="w-full bg-red-600 hover:bg-red-700 rounded-xl py-3 font-semibold transition flex items-center justify-center gap-2">
        ...
        @if(request()->anyFilled(['search', 'genre', 'status', 'sort']))
            <div class="mt-4 text-center">
                <a href="{{ route('films.index') }}" class="text-gray-400 hover:text-red-500 text-sm transition">
    ...
    @if(count($films) > 0)
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="flex gap-2">
                <button onclick="setView('grid')" id="gridViewBtn" class="p-2 rounded-lg bg-red-600 text-white">
                    <i class="fas fa-th-large"></i>
                </button>
                <button onclick="setView('list')" id="listViewBtn" class="p-2 rounded-lg bg-gray-700 text-gray-400 hover:bg-gray-600">
        ...
        <div id="gridView" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            ...
            <div class="group cursor-pointer" onclick="window.location='{{ route('films.show', $film->slug) }}'">
                ...
                <div>
                    <h3 class="text-white font-semibold group-hover:text-red-500 transition line-clamp-1">{{ $film->judul }}</h3>
                    ...
                    <p class="text-gray-400 text-xs mt-1">{{ $film->genre }} • {{ $film->duration }}</p>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div id="listView" class="hidden space-y-4">
            @foreach($films as $film)
            <div class="group cursor-pointer bg-gray-800/50 rounded-xl overflow-hidden border border-gray-700 hover:border-red-500 transition"
                ...
                <div class="flex flex-col md:flex-row gap-4 p-4">
                    <div class="w-full md:w-32 h-32 relative">
                        ...
                        @else
                            <div class="w-full h-full bg-gradient-to-br from-gray-700 to-gray-800 rounded-lg flex items-center justify-center">
                                <i class="fas fa-film text-3xl text-gray-500"></i>
                            </div>
                        @endif
                    </div>
                    <div class="flex-1">
                        <h3 class="text-xl font-bold text-white group-hover:text-red-500 transition">{{ $film->judul }}</h3>
                        <div class="flex items-center gap-3 mt-1">
                            ...
                            <span class="text-gray-400 text-sm">({{ number_format($film->rating_count / 1000, 1) }}k ulasan)</span>
                            <span class="text-gray-600">•</span>
                            <span class="text-gray-400 text-sm">{{ $film->genre }}</span>
                            <span class="text-gray-600">•</span>
                            <span class="text-gray-400 text-sm">{{ $film->duration }}</span>
                        </div>
                        <p class="text-gray-400 text-sm mt-2 line-clamp-2">{{ $film->synopsis }}</p>
                        <div class="flex items-center gap-3 mt-3">
                            <span class="bg-red-600/20 text-red-400 text-xs px-2 py-1 rounded">{{ $film->age_rating }}</span>
        ...
        <div class="text-center py-16">
            ...
            <h3 class="text-2xl font-bold text-white mb-2">Tidak ada film ditemukan</h3>
...
function setView(view) {
    ...
    if (view === 'grid') {
        ...
        gridBtn.classList.remove('bg-gray-700', 'text-gray-400');
        listBtn.classList.remove('bg-red-600', 'text-white');
        listBtn.classList.add('bg-gray-700', 'text-gray-400');
        localStorage.setItem('filmView', 'grid');
    } else {
        gridView.classList.add('hidden');
        listView.classList.remove('hidden');
        listBtn.classList.add('bg-red-600', 'text-white');
        listBtn.classList.remove('bg-gray-700', 'text-gray-400');
        gridBtn.classList.remove('bg-red-600', 'text-white');
        gridBtn.classList.add('bg-gray-700', 'text-gray-400');
